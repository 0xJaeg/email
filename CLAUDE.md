...
- `apps/web` — Next.js 16 (App Router, Turbopack, React 19) oversight dashboard. Routes: `/` (live ticket feed + stat cards), `/activity` (action log), `/approvals` (refund approval queue), `/users` (admin-only user management — create/update/delete with search + pagination), `/login` (email/password sign-in), `/no-access` (signed-in but no profile). The dashboard pages are async Server Components that read via the secret-key client (`lib/supabase/server.ts`, `server-only`); client components (`components/dashboard/tickets-table.tsx`, `components/dashboard/section-cards.tsx`) subscribe to Supabase Realtime via the publishable-key browser client (`lib/supabase/client.ts`) and refetch on change; `/activity` stays live via `components/activity/activity-realtime.tsx` (a `router.refresh()` island). Route protection is handled by `proxy.ts` (Next.js middleware). Query/mappers live in `lib/tickets.ts`; the paginated user list is in `lib/users.ts` and reuses the generic, URL-driven `components/shared/search-bar.tsx` + `components/shared/table-pagination.tsx` (`?query=`/`?page=`/`?size=`). User mutations (`lib/user-actions.ts`) re-check the caller's admin role and guard against self-delete / last-admin removal. `next.config.mjs` loads the repo-root `.env.local` via `@next/env` and transpiles `@workspace/ui` + `@workspace/db`. Dashboard components are organized by feature — `components/<feature>/` (`users/`, `approvals/`, `activity/`, `dashboard/`) for single-feature components, `components/shared/` for cross-feature primitives (search-bar, table-pagination, status-badges), and `components/layout/` for the app shell (sidebar, nav, header, theme-provider).
