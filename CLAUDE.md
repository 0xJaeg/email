...
**Pending verification**: slices C (Haiku classifier + caching) and D (refund decision tree + Sonnet escalation) are committed and pass typecheck/lint/boot, but no end-to-end test against a real inbound email has run yet — awaiting an Anthropic API key. Verify both together once the key lands; see `docs/initial-plan.md` and the slice C/D plan files for expected `decisions` and `audit_log` shapes.
