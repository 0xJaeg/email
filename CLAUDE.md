...
- `apps/web` — Next.js 16 (App Router, Turbopack, React 19) oversight dashboard. Routes: `/` (live ticket feed + stat cards), `/activity` (action log), `/approvals` (refund approval queue), `/login` (magic-link sign-in), `/no-access` (signed-in but no profile), `/auth/callback` (magic-link return). The dashboard pages are async Server Components that read via the secret-key client (`lib/supabase/server.ts`, `server-only`); client components (`tickets-table.tsx`, `activity-log.tsx`) subscribe to Supabase Realtime via the publishable-key browser client (`lib/supabase/client.ts`) and refetch on change. Route protection is handled by `proxy.ts` (Next.js middleware). Query/mappers live in `lib/tickets.ts`. `next.config.mjs` loads the repo-root `.env.local` via `@next/env` and transpiles `@workspace/ui` + `@workspace/db`.
