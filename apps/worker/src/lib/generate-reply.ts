...
export type GenerateReplyArgs = {
  ...
  productFacts?: string
  /** Customer-facing reply guidance (the node's editable prompt). */
...
export async function generateReply(
  ...
  const factsBlock = args.productFacts
    ...
    : ""
  const userMessage =
    `Write the customer-facing email reply for the message below. ` +
    `Plain text only — just the reply body.\n\n` +
